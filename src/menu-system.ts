...
export class MenuSystem {
  ...
  private async showServerInfo(): Promise<void> {
    ...
    if (this.currentServer.transport.type === "stdio") {
      ...
      this.currentServer.transport.type === "streamable-http" ||
      this.currentServer.transport.type === "sse"
    ) {
    ...
    const { transportType } = await inquirer.prompt([
      {
        ...
        choices: [
          ...
          {
            name: "🌐 Streamable HTTP (HTTP + SSE streaming)",
            value: "streamable-http",
          },
          {
            name: "⚠️  SSE (DEPRECATED - Server-Sent Events only)",
            value: "sse",
          },
          { name: "← Back", value: null },
        ],
  ...
  private async addStdioServer(): Promise<void> {
    const answers = await inquirer.prompt([
      ...
      },
      {
        ...
        validate: (input: string) =>
          input.trim().length > 0 || "Command is required",
      },
      {
        type: "input",
        name: "args",
    ...
    const config = {
      ...
      transport: {
        ...
        args: answers.args
          ? answers.args.split(" ").filter((a: string) => a.length > 0)
          : undefined,
      },
    };
